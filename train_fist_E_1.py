from .model.models import clean_siamese, SAFA, SAFA_semi, SAFA_delta, clean_siamese_delta
...
def validate(grd_descriptor, sat_descriptor):
    ...
    #grd_descriptor：A descriptor matrix for the ground image, with each row representing a feature vector of the ground image.
    #sat_descriptor：Descriptor matrix of satellite images, each row represents a feature vector of a satellite image.
    '''
...
def compute_loss(sat_global, grd_global, batch_hard_count=0, batch_size = 14, loss_weight = 10.0):
    ...
    with tf.name_scope('weighted_soft_margin_triplet_loss'):
        ...
        #If batch_hard_count is 0, perform a global comparison (full comparison). Transform the distance between all samples into a soft-margin ternary loss.
        if batch_hard_count == 0:
            ...
            loss_s2g = tf.reduce_sum(tf.log(1 + tf.exp(triplet_dist_s2g * loss_weight))) / pair_n#Calculation of weighted soft margin triple loss
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(start_epoch, start_epoch + number_of_epoch):
            ...
            while True:
               ...
               # print(distance_score(delta_list[:batch_size], delta_list[batch_size:]))
                if 'continuous' in mode:
                    if 'delta' in mode:
                ...
                if 'continuous' in mode:
                    loss_1_val,loss_3_val=sess.run([loss_1,loss_3],feed_dict=feed_dict)
                    loss_1_dic.append(loss_1_val)
                    loss_3_dic.append(loss_3_val)
                    
                    if 'delta' in mode:
                        loss_delta_val=sess.run(loss_delta,feed_dict=feed_dict)
                        loss_del_dic.append(loss_delta_val)
            ...
            if loss_1_dic:
                np.save(os.path.join(model_dir, f'loss_1_epoch{epoch}.npy'), loss_1_dic)
            if loss_3_dic:
                np.save(os.path.join(model_dir, f'loss_3_epoch{epoch}.npy'), loss_3_dic)
            if loss_del_dic:
                np.save(os.path.join(model_dir, f'loss_delta_epoch{epoch}.npy'), loss_del_dic)

            np.save(os.path.join(model_dir, f'loss_epoch{epoch}.npy'), loss_dic)
            # ---------------------- validation ----------------------
...
if __name__ == '__main__':
    ...
    tf.reset_default_graph()#recalculation chart

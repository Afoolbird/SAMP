...
def E_loss_2_2(heatmap,L,delta_2):
    # batch_size=heatmap.shape[0].value
    # heatmap=ut.post_process_and_normalize(heatmap)
    pixel_x_mean,pixel_y_mean=ut.get_spatial_mean(heatmap)
    x=tf.expand_dims(pixel_x_mean,1)
    y=tf.expand_dims(pixel_y_mean,1)
    estimated_location = tf.concat([x,y],axis=1)
    print('estimated_location shape:',estimated_location.shape)

    gt_x=delta_2[:,0]+320
    gt_y=319-delta_2[:,1]
    gt_x=tf.expand_dims(gt_x,1)
    gt_y=tf.expand_dims(gt_y,1)
    ground_truth_location=tf.concat([gt_x,gt_y],axis=1)
    print('ground_truth_location shape:',ground_truth_location.shape)

    Sigma=ut.get_sigma(L)

    # 计算行列式
    det_Sigma = Sigma[:, 0, 0] * Sigma[:, 1, 1] - Sigma[:, 0, 1] * Sigma[:, 1, 0] # batch_size x num_points   constants.EPSILON

    # 计算 Sigma 的逆矩阵
    Sigma_inv = tf.linalg.inv(Sigma)

    # 将 Sigma_inv 乘以 1/det_Sigma
    # Sigma_inverse = Sigma_inv / tf.reshape(det_Sigma, (batch_size, 1, 1))

    log_prob_1=0.5*tf.log(det_Sigma)
    log_prob_2=ut.get_mahalanobis_distance(ground_truth_location-estimated_location, Sigma_inv, ground_truth_location-estimated_location)

    loss=tf.reduce_mean(log_prob_1)+tf.reduce_mean(log_prob_2)
    # loss=loss*1e-3
...
def train(start_epoch=1, mode='', model_dir='', load_model_path='', load_mining_path='', break_iter=None,
    ...
    L_semi_delta=ut.get_uncertainty(L_semi)
    L_delta=ut.get_uncertainty(L)

    # choose loss
    with tf.name_scope('loss'):
        loss_0=compute_loss(sat_global, grd_global, batch_size=batch_size)
        loss_2_2=E_loss_2_2(heatmap_semi,L_semi,delta_2)*1e-4
        loss_2_2_pos=E_loss_2_2(heatmap,L,delta_1)*1e-3
        loss=loss_0+loss_2_2+loss_2_2_pos

        tf.summary.scalar('loss_0', loss_0)
        tf.summary.scalar('loss_2_2', loss_2_2)
        tf.summary.scalar('loss_2_2_pos', loss_2_2_pos)
        tf.summary.scalar('loss', loss)
    

    # set training
    rate = tf.train.exponential_decay(learning_rate_val, global_step, 910, 0.9)
    ...
    #variables_to_restore = [var for var in all_variables if 'vgg_sat'or'vgg_grd'or'spatial_grd'or'spatial_sat'or'train'or'Variable' in var.name]
    variables_to_restore = [var for var in all_variables if 'cholesky_block' not in var.name]
    variables_to_restore = [var for var in variables_to_restore if 'Variable' not in var.name]
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(start_epoch, start_epoch + number_of_epoch):
            ...
            while True:
                ...
                L_delta_val,L_delta_semi_val,loss_2_2_val,loss_2_2_val_pos,loss_0_val=sess.run([L_delta,L_semi_delta,loss_2_2,loss_2_2_pos,loss_0],feed_dict=feed_dict)
                L_semi_dic[iter]=np.mean(L_delta_semi_val)
                L_dic[iter]=np.mean(L_delta_val)

                
                print('loss_0:',loss_0_val)
                print('loss_2_2:',loss_2_2_val)
                print('loss_2_2_pos:',loss_2_2_val_pos)
            ...
            # ---------------------- validation ----------------------
                ...
                np.save(os.path.join(model_dir, f'L_semi_delta_{epoch}.npy'), L_semi_dic)
                np.save(os.path.join(model_dir, f'L_delta_{epoch}.npy'), L_dic)
...
if __name__ == '__main__':
    ...
    mining_start = 2
    number_of_epoch = 80
    learning_rate_val = 0.5*1e-5
    # learning_rate_val = 1e-5
    batch_size = 14
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
    os.environ["CUDA_VISIBLE_DEVICES"] = gpu_visible
    os.environ['TF_FORCE_GPU_ALLOW_GROWTH'] = 'true'
    model_load_dir = './data-E-2-1180/'
    # model_save_dir = './data-E-2-2-htp-relu-nor-xynor/'
    model_save_dir = './data-E-2-2/'
    load_mining_path = ''                        
